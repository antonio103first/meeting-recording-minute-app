...
# ── 요약 프롬프트 - 공식 양식 (텍스트) ──────────────────
_SUMMARY_FORMAL_TEXT_TEMPLATE = """너는 복잡한 회의 녹취록을 분석하여 핵심 정보를 체계적으로 정리하는 비즈니스 컨설턴트야.
제공된 텍스트의 모든 논의 사항을 놓치지 않으면서도, 읽기 쉽게 요약하여 일반 텍스트 형식으로 회의록을 작성해줘.

[작성 가이드라인]
1. 완결성: 회의에서 언급된 모든 수치, 업체명, 전략, 리스크 및 결정 사항을 빠짐없이 포함할 것.
2. 구조화: 주요 내용을 아래 위계에 따라 분류할 것. 마크다운 기호(#, *, ** 등)는 사용하지 말 것.
   * 1. 대제목
   * (1) 중제목
   * (A) 소제목 (필요 시 하위 불렛 포인트 활용)
3. 간결성: 문장은 서술형보다는 명사형 종결이나 '~함', '~임' 등의 간결한 문체를 사용할 것.
4. Action Item: 회의 중 언급된 담당자, 기한, 구체적인 실행 과제를 명확히 추출할 것.
...
5. Action Items
  ...
  No.1 | 액션: [내용] | 담당: [담당자] | 기한: [기한]
  No.2 | 액션: [내용] | 담당: [담당자] | 기한: [기한]

==============================
본 회의록은 녹취 텍스트를 기반으로 AI가 자동 작성하였습니다.
사실관계 확인이 필요한 사항은 추가 검토 바랍니다.
...
def summarize(stt_text: str, api_key: str, progress_cb=None,
    ...
    if summary_mode == "topic":
        ...
        template = _SUMMARY_FORMAL_TEXT_TEMPLATE
