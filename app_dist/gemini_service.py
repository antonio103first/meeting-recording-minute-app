...
2. 화자 구분: 위 코드로 화자를 명확히 표기. 직함·역할명 기재 금지
3. 데이터 정밀도: 모든 수치·금액·일자를 정확히 기록. 불확실한 경우 (추정) 표기
4. 추측 금지: 녹취록에 없는 내용은 절대 추가하지 않음. 불분명한 발언은 [불명확] 표기
5. Q&A 포착: 발표자 보고에 대해 다른 참석자가 질의·코멘트한 모든 교환을 반드시 Q&A 형식으로 기록
6. 구조: 화자별 보고 내용 → 각 보고 주제 하단에 해당 Q&A 배치
...
4. Q&A 포착 원칙 — 발표자 외 모든 참석자의 질의·코멘트를 빠짐없이 기록:
...
2. 데이터 정밀도: 수치·금액·일자 정확히 기록. 불확실하거나 추정한 경우 반드시 (추정) 표기
3. 추측 금지: 녹취록에 없는 내용은 절대 추가하지 않음. 다만 맥락상 불가피하게 추정이 필요한 경우 반드시 (추정) 표기
4. 보완 주석: 대표 발언 중 기술적·사업적 맥락이 필요한 곳에 *(주석: ...)* 형태로 인라인 삽입
   - 기술 주석: 이 기술이 왜 중요한가 / 대안은 뭔가 / 경쟁사도 할 수 있는가 / 기술적 한계는
   - 사업 주석: 이 숫자가 큰 건가 작은 건가 / 시장 맥락에서 어떤 의미인가 / 비교 기준은
   - 검증 주석: 이 주장은 검증됐는가 / IR 자료와 일치하는가
5. 전문 용어: 첫 출현 시 괄호 안에 설명 추가. 예) EPM(Electro Permanent Magnet, 전자영구자석)
   동일 용어는 첫 출현에만 설명. 너무 일반적인 용어(AI, SW, HW 등)는 설명 불필요.
6. STT 오인식: 기업명·인명이 어색하면 *(STT 오인식 의심)* 표기 후 IR 자료와 대조
7. IR 자료 vs 녹취록 수치 불일치: IR 자료 우선 채택 + *(녹취록 "X" → IR 자료 기준 "Y" 교정)* 주석

[기술 해설 인용 블록 — 투자 판단에 크리티컬한 기술 2~3개에만 집중]
> 인용 블록 형태로 작성. 아래 3가지 유형 중 해당하는 것만 선택:

[유형 1] 핵심 기술 해설 — "이 기술이 뭔지, 왜 어려운지"
- 쉬운 비유로 기술 개념을 직관적으로 설명
- 해당 기술이 실제로 하는 일을 단계별(1→2→3)로 서술
- 이 기술의 난이도가 어디에 있는지, 경쟁사는 왜 못 하는지
- 해당 기업의 차별점: 경쟁사 대비 어떻게 차별화했는지, 투자 관점에서 왜 의미 있는지
제목 형식: > **[기술명]란 무엇인가 — 기술 해설**

[유형 2] 경쟁 기술 비교 해설 — "이 기술의 대안은 뭐고, 왜 안 되는지"
- 경쟁 기술의 원리를 쉬운 비유로 설명
- 장점은 인정하되 현실적 한계(양산성·가격·성숙도)를 명확히
- 왜 주류가 안 되는가: 기술적 문제 + 업계 사례
제목 형식: > **왜 [경쟁기술방식]이 아닌가 — 경쟁 기술 비교**

[유형 3] 기술 선택의 합리성 — "왜 이 기술 방식을 선택했는가"
- 비교표 요약 + 시장 수요·양산 현실성·가격 경쟁력 관점에서 선택 이유
제목 형식: > **왜 [기술방식]인가 — 기술 선택의 구조적 이유**

[기술 해자 냉정 평가 — 회사 핵심 기술 주장 1~2개에만 적용]
제목 형식: > **[기술적 주석] "[회사 주장 요약]"의 실질적 의미 — 냉정한 평가**
(1) 주장 분해: 이 성능이 소자/기술 자체의 고유 특성인가, 이 회사가 개발한 것인가
    예) "1만 배 감도" → SiPM 소자 고유 특성(회사 발명 아님) vs "32채널 단일 모듈" → 시스템 엔지니어링(의미 있음)
(2) 진짜 가치: 상용 부품 조립인가 시스템 레벨 엔지니어링인가, 2~3년 축적이 필요한가
(3) 경쟁 경로: 동일 기술 경로로 따라올 수 있는가, 대체 기술 경로가 이미 있는가, 대기업 진입 가능성
(4) 해자 내구성: 구조적 해자(특허+독점소싱+양산실적+고객lock-in 복합) / 시간 우위(1~2년) / 약한 해자 중 판정 + 근거
(5) 결론: "[주장]은 [소자/기술] 고유 특성이지 이 회사의 발명이 아님. 그러나 [진짜 가치]는 의미가 있음. 해자는 [판정]으로 판단함." 형태로 1~2문장 요약
...
**Q [케이런]** (질의 내용)
**A [IR회사명]** (답변 내용) *(미팅에서 처음 확인 / IR 자료)* — 답변 톤: [자신있게 즉답함 / 머뭇거린 뒤 / 회피]

(Q&A 최소 2~3개. Q/A는 각각 별도 줄, 각 Q&A 블록 사이 빈 줄 1줄로 구분)
...
**이 섹션이 미팅노트에서 가장 밀도가 높아야 함. IR Deck 내용 재탕 금지. 미팅 Q&A로 확인된 기술의 실체 중심으로 작성.**
**구성 순서: 제품 포트폴리오 테이블 → 핵심 기술 Q&A → 경쟁사 기술 비교표 → 기술 해설 인용 블록 → 기술 해자 냉정 평가**
...
> **[기술명]란 무엇인가 — 기술 해설**
> (유형 1·2·3 중 해당하는 것만 작성)

> **[기술적 주석] "[회사 주장]"의 실질적 의미 — 냉정한 평가**
> (1) 주장 분해 → (2) 진짜 가치 → (3) 경쟁 경로 → (4) 해자 내구성 → (5) 결론

**Q [케이런]** (기술 관련 질의)
**A [IR회사명]** (기술 관련 답변) *(미팅에서 처음 확인 / IR 자료)* — 답변 톤: [자신있게 즉답함 / 머뭇거린 뒤 / 회피]

(Q&A 최소 2~3개. Q/A 각각 별도 줄, 각 Q&A 사이 빈 줄 1줄)
...
파이프라인 확정성 분류:
- **확정**: [고객명] — [계약 현황] *(미팅에서 처음 확인 / IR 자료)*
- **높은 확률**: [고객명] — [근거]
- **미확정**: [고객명] — [상태]

**Q [케이런]** (고객·계약 관련 질의)
**A [IR회사명]** (답변) *(미팅에서 처음 확인 / IR 자료)* — 답변 톤: [자신있게 즉답함 / 머뭇거린 뒤 / 회피]
...
④ IR Deck에 없는 새로운 정보를 IR회사가 공개한 지점 *(미팅에서 처음 확인)* 태그
⑤ 밸류에이션·딜 구조·펀딩 현황에 관한 논의

### [논점 제목]
- **논의 내용**:
  **Q [케이런]** 질의 또는 반론 내용을 한 문장으로 서술.
  **A [IR회사명]** 답변 내용 서술. — 답변 톤: "자신있게 즉답함" / "잠시 머뭇거린 뒤 답변함" / [답변 회피] / [구체 답변 불가]
  *(미팅에서 처음 확인 / IR 자료 / Pre-Screening 우려 → 해소·미해소)*
...
**[긍정 시그널]**
- (미팅에서 확인된 강점·차별점 — *(미팅에서 처음 확인)* 항목 중심)
