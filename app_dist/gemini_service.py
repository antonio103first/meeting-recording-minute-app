...
[작성 원칙]
1. 팩트 기반: 대화에서 실제 나눈 내용에만 기초하여 작성. 녹취에 없는 내용 절대 추가 금지
2. **서술 전용 — 질문·답변 문장 자체를 쓰지 않음**: `Q [화자]` / `A [화자]` 나열 형식을 쓰지 않음. 이 메모는 통화에 참여하지 않은 사람이 읽는 기록이므로 "누가 무엇을 물었는지"가 아니라 "무엇이 사실이고 무엇이 확인됐는지"만 중요함. **"[화자]는 ~질의함/문의함", "~는 ~답함/답변함"처럼 화자 뒤에 질문·답변 동작을 서술하는 문장을 아예 쓰지 않음.** 대신 질문으로 확인하려던 내용과 답변에서 나온 사실을 통째로 묶어, 처음부터 그 사실이 확정된 것처럼 서술함. 확인이 여러 차례(3~4회) 이어지더라도 전부 예외 없이 하나의 문단에 흡수함. **단, 정보 손실은 절대 안 됨**: 질문·답변에 담긴 사실(수치·조건·결정)은 하나도 빠짐없이 담되, "누가 물었다/답했다"는 표현 형식만 없앰. 실제 질문이 불분명한데 억지로 Q&A가 오간 것처럼 창작하지 않음.
...
6. **서술 전용 — 질문·답변 문장 자체를 쓰지 않음**: 강의 중 질의응답이 있는 경우 `[Q]`/`[A]` 나열 형식을 쓰지 않습니다. 이 노트는 강의를 듣지 않은 사람이 읽는 기록이므로 "누가 무엇을 물었는지"가 아니라 "무엇이 설명됐는지"만 중요합니다. **"한 참석자가 ~질문함", "강사는 ~답함"처럼 질문·답변 동작을 서술하는 문장을 아예 쓰지 않습니다.** 대신 질문으로 확인하려던 내용과 답변에서 나온 설명을 통째로 묶어 해당 소주제 서술 안에 자연스럽게 흡수합니다. 질문이 여러 차례(3~4회) 이어지더라도 전부 예외 없이 하나의 문단에 흡수합니다. **단, 정보 손실은 절대 안 됩니다**: 질문·답변에 담긴 내용은 하나도 빠짐없이 담되, "누가 물었다/답했다"는 표현 형식만 없앱니다. 실제 질문이 불분명한데 억지로 만들어내지 않습니다.
...
8. **서술 전용 — 질문·답변 문장 자체를 쓰지 않음**: 케이런과 상대방 사이에 의미 있는 질의·응답·이견이 오간 핵심 쟁점·확인사항·중요 의사결정이 있으면, `Q [화자]`/`A [화자]` 나열 형식이 아니라 해당 주제 소항목 서술(또는 **논의 및 확인사항** 레이블)에 녹여 정리함. 이 회의록은 미팅에 참석하지 않은 사람이 읽는 기록이므로 "누가 무엇을 물었는지"가 아니라 "무엇이 사실이고 무엇이 확인됐는지"만 중요함. **"[화자]는 ~질의함/문의함", "~는 ~답함/답변함"처럼 화자 뒤에 질문·답변 동작을 서술하는 문장을 아예 쓰지 않음.** 대신 질문으로 확인하려던 내용과 답변에서 나온 사실을 통째로 묶어, 처음부터 그 사실이 확정된 것처럼 서술함. 확인이 여러 차례(3~4회) 이어지더라도 전부 예외 없이 하나의 문단에 흡수함. **단, 정보 손실은 절대 안 됨**: 질문·답변에 담긴 사실(수치·조건·결정)은 하나도 빠짐없이 담되, "누가 물었다/답했다"는 표현 형식만 없앰. STT에 실제 질문이 분명하지 않은데 억지로 만들어내지 않음.
