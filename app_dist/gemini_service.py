...
[작성 원칙]
1. 팩트 기반: 대화에서 실제 나눈 내용에만 기초하여 작성. 녹취에 없는 내용 절대 추가 금지
2. **Q&A 대화 형식 금지, 서술로 통합**: `Q [화자]` / `A [화자]` 나열 형식을 쓰지 않음. 오간 질문·답변·보충 설명은 순서 그대로 옮기지 말고 **하나의 서술 문단**으로 녹여 정리함(발언자·발언 내용은 가능한 인용). 실제 질문이 불분명한데 억지로 Q&A가 오간 것처럼 창작하지 않음.
...
8. 문체: 모든 서술 문장은 "~음" 종결 (예: "진행 중임", "완료함")
...
6. **Q&A 대화 형식 금지, 서술로 통합** — 강의 중 질의응답이 있는 경우 `[Q]`/`[A]` 나열 형식을 쓰지 않고, 오간 질문·답변을 해당 소주제 서술 안에 자연스럽게 녹여 정리합니다(예: "이에 대해 한 참석자가 ~라고 질문했고, 강사는 ~라고 답함"). 실제 질문이 불분명한데 억지로 만들어내지 않습니다.
...
8. **Q&A 대화 형식 금지, 서술로 통합**: 케이런과 상대방 사이에 의미 있는 질의·응답·이견이 오간 핵심 쟁점·확인사항·중요 의사결정이 있으면, `Q [화자]`/`A [화자]` 나열 형식이 아니라 해당 주제 소항목 서술(또는 **논의 및 확인사항** 레이블)에 자연스러운 문장으로 녹여 정리함. STT에 실제 질문이 분명하지 않은데 억지로 만들어내지 않음
