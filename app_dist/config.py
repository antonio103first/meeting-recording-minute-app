...
def reload_paths():
    ...
    mp3_full_path / stt_full_path / sum_full_path 가 설정되면 독립 경로 사용,
    없으면 recording_dir + 서브폴더명 사용 (하위 호환)
    """
    ...
    MP3_SAVE_DIR     = Path(cfg["mp3_full_path"]) if cfg.get("mp3_full_path") \
                       else RECORDING_BASE / mp3_sub
    STT_SAVE_DIR     = Path(cfg["stt_full_path"]) if cfg.get("stt_full_path") \
                       else RECORDING_BASE / stt_sub
    SUMMARY_SAVE_DIR = Path(cfg["sum_full_path"]) if cfg.get("sum_full_path") \
                       else RECORDING_BASE / summary_sub
