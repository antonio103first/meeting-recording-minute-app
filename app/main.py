"""
회의녹음요약 - 메인 GUI (tkinter)
탭 구성: 녹음/변환 | 회의목록 | 설정
자동화 파이프라인: STT → (화자이름) → 요약 → 저장 → Drive
"""
import sys
import os
import re
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._current_mp3   = None   # 현재 녹음/선택된 파일 경로
        self._stt_text      = ""
        self._summary_text  = ""
    ...
    def _build_tab_list(self):
        ...
        col_w   = {"날짜": 140, "파일명": 220, "녹음": 60,
                   "STT":  60,  "요약":  60,  "Drive": 80}
        col_min = {"날짜":  80, "파일명": 100, "녹음": 40,
                   "STT":  40,  "요약":  40,  "Drive": 50}
        for c in cols:
        ...
        paned.add(tree_frame, weight=3)   # 트리뷰 : 내용보기 = 3 : 1
    ...
    def _build_tab_settings(self):
        ...
        tk.Label(gem_card, textvariable=self._gem_status_var,
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack()
        tk.Label(gem_card,
                 text="▶ Google AI Studio (aistudio.google.com) 에서 무료 API 키를 발급받으세요.",
        ...
        clova_id_row.pack(fill="x", pady=4)
        ...
        clova_sec_row.pack(fill="x", pady=4)
        self._clova_sec_var = tk.StringVar(value=self._cfg.get("clova_secret_key", ""))
        self._clova_sec_entry = tk.Entry(
            clova_sec_row, textvariable=self._clova_sec_var,
            width=52, font=FONT_BODY, show="*")
        self._clova_sec_entry.pack(side="left")
        self._clova_sec_show = False
        ...
        # STT 엔진 선택
        engine_frm = tk.LabelFrame(clova_card, text="  기본 STT 엔진  ", font=FONT_SMALL,
                                   bg=CARD_BG, fg=TEXT, padx=8, pady=4)
        engine_frm.pack(fill="x", pady=(8, 0))
        self._stt_engine_var = tk.StringVar(value=self._cfg.get("stt_engine", "gemini"))
        tk.Radiobutton(engine_frm, text="Gemini (Google)",
                       variable=self._stt_engine_var, value="gemini",
                       bg=CARD_BG, font=FONT_BODY, activebackground=CARD_BG,
                       command=self._save_stt_engine).pack(side="left", padx=12)
        tk.Radiobutton(engine_frm, text="CLOVA Speech (NAVER) ★권장★",
                       variable=self._stt_engine_var, value="clova",
                       bg=CARD_BG, font=FONT_BODY, activebackground=CARD_BG,
                       command=self._save_stt_engine).pack(side="left", padx=12)

        tk.Label(clova_card,
                 text="▶ NAVER Cloud Console → AI·NAVER API → CLOVA Speech에서 앱을 생성하고 키를 발급받으세요.",
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack(anchor="w", pady=(8, 0))
        ...
        # ─ Google Drive ─────────────────────────────────
        self._card(inner, "☁ Google Drive 설정").pack(fill="x", **pad)
        gd_card = self._last_card

        self._gd_status_var = tk.StringVar(value="")
        self._update_gd_status()
        tk.Label(gd_card, textvariable=self._gd_status_var,
                 font=FONT_BODY, bg=CARD_BG, fg=TEXT).pack(anchor="w", pady=(0, 8))

        tk.Label(gd_card, text="OAuth 클라이언트 JSON 파일:",
                 font=FONT_BODY, bg=CARD_BG, fg=TEXT).pack(anchor="w")
        oauth_row = tk.Frame(gd_card, bg=CARD_BG)
        oauth_row.pack(fill="x", pady=4)
        self._oauth_path_var = tk.StringVar(value="")
        tk.Entry(oauth_row, textvariable=self._oauth_path_var,
                 width=44, font=FONT_SMALL,
                 state="readonly").pack(side="left")
        self._btn(oauth_row, "📂 파일 선택", TEXT_LIGHT,
                  self._pick_oauth_file, w=12).pack(side="left", padx=4)
        self._btn(oauth_row, "등록", ACCENT,
                  self._register_oauth, w=6).pack(side="left", padx=2)

        auth_row = tk.Frame(gd_card, bg=CARD_BG)
        auth_row.pack(pady=8)
        self._btn(auth_row, "🔐 Google 인증 (브라우저)", ACCENT,
                  self._auth_drive, w=22).pack(side="left", padx=4)
        self._btn(auth_row, "연결 해제", DANGER,
                  self._revoke_drive, w=10).pack(side="left", padx=4)

        self._gd_msg_var = tk.StringVar(value="")
        tk.Label(gd_card, textvariable=self._gd_msg_var,
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack()
        tk.Label(gd_card,
                 text="▶ Google Cloud Console에서 OAuth 2.0 클라이언트 ID(데스크톱 앱)를 생성하세요.",
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack(anchor="w", pady=(4, 0))

        # 업로드 폴더 설정
        ttk.Separator(gd_card).pack(fill="x", pady=(12, 6))
        tk.Label(gd_card, text="📁 업로드 폴더 설정", font=FONT_H2,
                 bg=CARD_BG, fg=TEXT).pack(anchor="w")

        # MP3 폴더
        mp3f_row = tk.Frame(gd_card, bg=CARD_BG)
        mp3f_row.pack(fill="x", pady=(6, 2))
        tk.Label(mp3f_row, text="녹음 폴더명:", font=FONT_BODY,
                 bg=CARD_BG, fg=TEXT, width=12, anchor="w").pack(side="left")
        self._drive_mp3_name_var = tk.StringVar(
            value=self._cfg.get("drive_mp3_folder_name", "녹음파일"))
        tk.Entry(mp3f_row, textvariable=self._drive_mp3_name_var,
                 width=22, font=FONT_BODY).pack(side="left", padx=4)
        self._btn(mp3f_row, "📁 생성/찾기", ACCENT,
                  self._ensure_mp3_folder, w=10).pack(side="left", padx=4)
        self._drive_mp3_id_var = tk.StringVar(
            value=self._cfg.get("drive_mp3_folder_id", "(미설정)") or "(미설정)")
        tk.Label(mp3f_row, textvariable=self._drive_mp3_id_var,
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack(side="left", padx=4)

        # TXT 폴더
        txtf_row = tk.Frame(gd_card, bg=CARD_BG)
        txtf_row.pack(fill="x", pady=(2, 6))
        tk.Label(txtf_row, text="회의록 폴더명:", font=FONT_BODY,
                 bg=CARD_BG, fg=TEXT, width=12, anchor="w").pack(side="left")
        self._drive_txt_name_var = tk.StringVar(
            value=self._cfg.get("drive_txt_folder_name", "회의록(요약)"))
        tk.Entry(txtf_row, textvariable=self._drive_txt_name_var,
                 width=22, font=FONT_BODY).pack(side="left", padx=4)
        self._btn(txtf_row, "📁 생성/찾기", ACCENT,
                  self._ensure_txt_folder, w=10).pack(side="left", padx=4)
        self._drive_txt_id_var = tk.StringVar(
            value=self._cfg.get("drive_txt_folder_id", "(미설정)") or "(미설정)")
        tk.Label(txtf_row, textvariable=self._drive_txt_id_var,
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack(side="left", padx=4)

        both_row = tk.Frame(gd_card, bg=CARD_BG)
        both_row.pack(pady=4)
        self._btn(both_row, "🚀 두 폴더 한번에 생성", SUCCESS,
                  self._ensure_both_folders, w=22).pack(side="left", padx=4)
        self._drive_folder_msg_var = tk.StringVar(value="")
        tk.Label(gd_card, textvariable=self._drive_folder_msg_var,
                 font=FONT_SMALL, bg=CARD_BG, fg=TEXT_LIGHT).pack()

        # ─ 저장 경로 ────────────────────────────────────
        self._card(inner, "📁 저장 경로 정보").pack(fill="x", **pad)
        path_card = self._last_card
        for label, path in [
            ("녹음파일:", config.AUDIO_SAVE_DIR),
            ("회의록(요약):", config.SUMMARY_SAVE_DIR),
            ("앱 데이터:", config.APP_DATA_DIR),
        ]:
            row = tk.Frame(path_card, bg=CARD_BG)
            row.pack(fill="x", pady=2)
            tk.Label(row, text=label, font=FONT_BODY,
                     bg=CARD_BG, fg=TEXT, width=14, anchor="w").pack(side="left")
            tk.Label(row, text=str(path), font=FONT_SMALL,
    ...
    def _start_pipeline(self):
        ...
        tk.Label(dlg, text="변환 옵션 선택", font=FONT_H2,
                 bg=CARD_BG, fg=TEXT).pack(pady=(18, 4))
        ...
        frm_stt.pack(fill="x", padx=20, pady=(4, 0))
        ...
        frm1.pack(fill="x", padx=20, pady=8)
        ...
        stt_engine_now = self._pipeline_stt_engine

        def run_stt():
            if stt_engine_now == "clova":
                cid     = self._cfg.get("clova_invoke_url", "").strip()
                csecret = self._cfg.get("clova_secret_key", "").strip()
                ok, text = clova.transcribe(
                    self._current_mp3, cid, csecret,
                ...
                ok, text = gemini.transcribe(
                    ...
                    progress_cb=lambda v: self.after(0, lambda: self._set_prog(self._stt_prog, v)),
                    num_speakers=num_spk,
                    speaker_names={},   # 이름 치환은 파이프라인에서 별도 처리
                    cancel_event=self._cancel_event,
                    status_cb=lambda msg: self.after(0, lambda: self._stt_status_var.set(msg)),
                )
    ...
    def _show_speaker_rename_dialog(self, stt_text: str):
        """③ 화자 이름 입력 다이얼로그 (STT 텍스트에서 [화자N] 자동 감지)"""
        # STT 텍스트에서 [화자N] 패턴 추출
        found = sorted(set(re.findall(r'\[화자(\d+)\]', stt_text)), key=int)
        if not found:
            # [화자N] 없으면 화자 수 기반으로 생성
            n = self._spk_var.get()
    ...
    def _on_pipeline_summary_done(self, ok: bool, text: str, stt_text: str):
        ...
        if stt_text:
            ok2, path = fm.save_stt_text(stt_text, str(config.AUDIO_SAVE_DIR), save_name + "_녹음")
            if ok2:
                stt_path = path
        ...
        if text:
            ok3, path = fm.save_summary_text(text, str(config.SUMMARY_SAVE_DIR), save_name + "_요약")
            if ok3:
                sum_path = path
        ...
        if gdrive.is_available():
            status = gdrive.get_credentials_status()
            if status["status"] == "authenticated":
                # 폴더 ID 조회 — 없으면 자동 생성
                mp3_fid = self._cfg.get("drive_mp3_folder_id", "").strip()
                txt_fid = self._cfg.get("drive_txt_folder_id", "").strip()
                if not mp3_fid:
                    mp3_name = self._cfg.get("drive_mp3_folder_name", "녹음파일")
                    ok_f, mp3_fid, msg_f = gdrive.ensure_folder(mp3_name)
                    if ok_f:
                        self._cfg["drive_mp3_folder_id"] = mp3_fid
                        config.save_config(self._cfg)
                    else:
                        drive_msgs.append(f"❌ MP3 폴더 생성 실패: {msg_f}")
                if not txt_fid:
                    txt_name = self._cfg.get("drive_txt_folder_name", "회의록(요약)")
                    ok_f, txt_fid, msg_f = gdrive.ensure_folder(txt_name)
                    if ok_f:
                        self._cfg["drive_txt_folder_id"] = txt_fid
                        config.save_config(self._cfg)
                    else:
                        drive_msgs.append(f"❌ TXT 폴더 생성 실패: {msg_f}")

                results = gdrive.upload_meeting_files(
                    mp3_path  or "",
                    stt_path  or "",
                    sum_path  or "",
                    mp3_folder_id=mp3_fid,
                    txt_folder_id=txt_fid,
                )
                label_map = {"mp3": "녹음(MP3)", "stt": "STT 텍스트", "summary": "요약 회의록"}
                for key, info in results.items():
                    if info["ok"]:
                        drive_msgs.append(f"✅ Drive {label_map[key]}: {info['link']}")
                        drive_ok = True
                    elif info["msg"] != "파일 없음":
                        drive_msgs.append(f"❌ Drive {label_map[key]} 실패: {info['msg']}")
            else:
                drive_msgs.append("ℹ Drive 미연결 — 로컬 저장만 완료됨")
        else:
            drive_msgs.append("ℹ google-auth 패키지 없음 — 로컬 저장만 완료됨")

        all_msgs = msgs + drive_msgs
        self._save_status_var.set(" | ".join(all_msgs))

        # 완료 팝업
        result_lines = ["🎉 파이프라인 완료!", ""]
        result_lines += msgs
        if drive_msgs:
            result_lines.append("")
            result_lines += drive_msgs
    ...
    def _pick_oauth_file(self):
        path = filedialog.askopenfilename(
            title="OAuth 클라이언트 JSON 선택",
            filetypes=[("JSON 파일", "*.json"), ("모든 파일", "*.*")])
        if path:
            self._oauth_path_var.set(path)

    def _register_oauth(self):
        path = self._oauth_path_var.get().strip()
        if not path:
            messagebox.showwarning("알림", "파일을 먼저 선택해주세요.")
            return
        ok, msg = gdrive.save_credentials_file(path)
        self._gd_msg_var.set(("✅ " if ok else "❌ ") + msg)
        self._update_gd_status()

    def _auth_drive(self):
        self._gd_msg_var.set("브라우저에서 인증 진행 중...")
        self.update()
        ok, msg = gdrive.authenticate()
        self._gd_msg_var.set(("✅ " if ok else "❌ ") + msg)
        self._update_gd_status()

    def _revoke_drive(self):
    ...
    def _toggle_clova_secret_vis(self):
        self._clova_sec_show = not self._clova_sec_show
        self._clova_sec_entry.config(show="" if self._clova_sec_show else "*")

    def _save_clova_keys(self):
        self._cfg["clova_invoke_url"]     = self._clova_id_var.get().strip()
        self._cfg["clova_secret_key"] = self._clova_sec_var.get().strip()
        config.save_config(self._cfg)
    ...
    def _test_clova(self):
        ...
        csecret = self._clova_sec_var.get().strip()
        if not cid or not csecret:
            messagebox.showwarning("알림", "Invoke URL과 Secret Key를 입력해주세요.")
            return
    ...
    def _ensure_mp3_folder(self):
        name = self._drive_mp3_name_var.get().strip() or "녹음파일"
        self._drive_folder_msg_var.set(f"'{name}' 폴더 처리 중...")
        self.update()
        ...
        if ok:
            ...
            config.save_config(self._cfg)
            self._drive_mp3_id_var.set(f"ID: {fid[:16]}...")
            self._drive_folder_msg_var.set(f"✅ {msg}")
        else:
            self._drive_folder_msg_var.set(f"❌ {msg}")

    def _ensure_txt_folder(self):
        name = self._drive_txt_name_var.get().strip() or "회의록(요약)"
        self._drive_folder_msg_var.set(f"'{name}' 폴더 처리 중...")
        self.update()
        ok, fid, msg = gdrive.ensure_folder(name)
        if ok:
            ...
            config.save_config(self._cfg)
            self._drive_txt_id_var.set(f"ID: {fid[:16]}...")
            self._drive_folder_msg_var.set(f"✅ {msg}")
        else:
            self._drive_folder_msg_var.set(f"❌ {msg}")

    def _ensure_both_folders(self):
        mp3_name = self._drive_mp3_name_var.get().strip() or "녹음파일"
        txt_name = self._drive_txt_name_var.get().strip() or "회의록(요약)"
        self._drive_folder_msg_var.set("두 폴더 생성 중...")
        self.update()
        res = gdrive.init_drive_folders(mp3_name, txt_name)
        msgs = []
        if res["mp3_ok"]:
            self._cfg["drive_mp3_folder_name"] = mp3_name
            self._cfg["drive_mp3_folder_id"]   = res["mp3_id"]
            self._drive_mp3_id_var.set(f"ID: {res['mp3_id'][:16]}...")
            msgs.append(f"MP3: {res['mp3_msg']}")
        else:
            msgs.append(f"MP3 실패: {res['mp3_msg']}")
        if res["txt_ok"]:
            self._cfg["drive_txt_folder_name"] = txt_name
            self._cfg["drive_txt_folder_id"]   = res["txt_id"]
            self._drive_txt_id_var.set(f"ID: {res['txt_id'][:16]}...")
            msgs.append(f"TXT: {res['txt_msg']}")
        else:
            msgs.append(f"TXT 실패: {res['txt_msg']}")
        config.save_config(self._cfg)
        self._drive_folder_msg_var.set(" | ".join(msgs))

    def _update_gd_status(self):
        status = gdrive.get_credentials_status()
        icon = {"authenticated": "🟢", "need_auth": "🟡", "no_credentials": "🔴"}.get(
            status["status"], "⚪")
        self._gd_status_var.set(f"{icon} 상태: {status['msg']}")
