...
- **현재 버전**: v4.0.4 (2026-08-31 주간회의록 등 Q&A 서술화 규칙 원점 재설계 — 실사용 검증 대기)
...
| 버전 | 주요 변경 |
|------|----------|
| **v4.0.4** | **Q&A 서술화 규칙 5차 재설계 — 프롬프트 지시만으로는 계속 재발, 결국 "화자코드+질의/답변 동사 금지"로 원점 재설계.** v4.0.3(Q&A 대화 블록 폐지)을 배포한 뒤, 회원이 실제 재요약 결과를 4차례 연속 제시하며 지적 — 알테오젠 위약벌/주매청 협의, Affini 보호예수, 위밋모빌리티 M&A 등 **한 사람이 딜 구조를 캐묻는 실사용 시나리오**에서 문단 앞부분은 잘 정리되다가 뒷부분(후속 확인질문 3~4개)만 계속 "[K1]은 ~질의함. [K2]는 ~답함." 식으로 되돌아가는 동일 패턴이 재현됨. 대응 이력: ①ORG 양식의 ❌/✅ 예시 패턴을 나머지 7개 양식(IR 제외)에 이식 → 실패. ②예시를 실제 실패 사례와 거의 동일한 다회차(3~4회) Q&A로 교체 + "질문 3~4회 이어져도 예외 없이 흡수" 문구 추가 → 실패(재빌드 후 재현 확인). ③원인 재진단 — SPEAKER·TOPIC 두 양식에만 남아있던 "다른 참석자 질의·이견을 **빠짐없이 포착**하되"라는 문구가 "재구성하라"는 지시와 충돌해, 모델이 "혹시 빠뜨릴까 봐" 뒷부분을 안전하게 원본 Q&A로 남기는 것으로 추정 → "포착"의 의미를 "답변 속 사실 정보"로 재정의 + "부분 준수 금지" 규칙 추가 → 역시 실패(위밋모빌리티 사례로 4번째 재현, 회원이 "왜 계속 이래?"). ④후처리 2차 LLM 호출(정규식으로 Q&A 잔존 패턴 감지 → 좁은 범위 재작성 프롬프트로 교정)을 제안했으나 회원이 지연·비용 문제로 반려, "프롬프트를 원점에서 개선"하도록 지시. ⑤**최종 재설계**: 그간 누적되어 서로 겹치던 규칙 3~4개(Q&A금지·다화자통합·부분준수금지)를 SPEAKER·TOPIC에서 하나로 통합하고, "쟁점 중심으로 재구성하라"는 **추상적 지시**를 "**화자 코드 바로 뒤에 '질의/질문/문의', '답변/답함' 동사를 붙인 문장을 아예 쓰지 마라**"는 **단어 수준의 기계적 금지**로 전환 — 스타일 판단이 아니라 단순 패턴 매칭 수준이라 모델이 지키기 쉬울 것으로 기대. 회원이 "빠짐없이 기록하는 건 아주 중요하다"고 명시적으로 확인해줘서, 완전성 요구는 절대 약화시키지 않고 "포착 대상은 사실 정보이지 Q&A 형식이 아니다"를 규칙 최상단에 못박음. 7개 양식(IR 제외) 전부 동일 톤으로 재작성 + PC·모바일 동기화 + 검증(`ast.parse`/`compileDebugKotlin`) + exe·APK 재빌드·재설치까지 완료. **미해결(중요)**: 이 5차 시도가 실제로 재발을 막는지는 **아직 실사용으로 검증되지 않음** — 회원이 "일단 여기서 마무리하고 써보고 수정하겠다"며 직접 테스트 후 필요시 추가 조정하기로 함. 이전 4번의 시도가 전부 처음엔 그럴듯해 보였다가 재현됐으므로, 이번 것도 낙관하지 말고 다음 세션에서 회원의 실사용 피드백으로 재확인 필요.
| **v4.0.3** | **주간회의록(speaker) Q&A 대화 블록 전면 폐지 + 동일 화자 연속 다건 보고 통합. PC·모바일 동시 반영.** 배경: 회원이 파트너 주간회의 실제 사례([K2]가 모티프·PK·어피닛·에이치앤파워·GL캠·모라이 6개 포트폴리오사 현황을 연달아 보고)를 제시하며, 기존 규칙(주제=회사마다 `###` 소제목을 새로 만들고 매번 [K2] 화자 코드를 반복)이 한 사람이 순서대로 브리핑한 내용을 부자연스럽게 쪼갠다고 지적. ① **Q&A 대화 블록 폐지** — 주간회의록은 v3.0.6부터 유지해 온 `Q [코드]`/`A [코드]` blockquote 형식(작성 원칙 6·7, 별도 `[Q&A 형식]` 절)을 완전히 제거하고, 질의·답변을 주제 서술 문단 안에 "이에 대해 [K3]는 ~라고 질의했고 [K1]은 ~라고 답함" 형태로 녹여 통합 — v4.0.1에서 topic·formal_md·lecture_md·conference·org 5종에 이미 적용한 것을 스피커 양식에도 마저 통일(스피커·IR미팅만 예외로 남아 있던 것 해소, IR미팅은 미변경 유지). ② **동일 화자 연속 다건 보고 통합** — 한 화자가 같은 발언 구간에서 여러 안건을 이어서 보고하면 안건별 소제목·화자 반복 없이, 포괄적 소제목 하나 + "(보고: [K?])" 1회 표기 + **안건명** — 내용 형식의 굵은 리드로 빈 줄 없이 나열(4가지 안 중 회원이 "굵은 회사명 리드 + 리스트업, 빈 줄 없음" 안을 선택). [K1]/[K2]/[K3]/[S1] 모든 화자에 동일 적용, 서로 다른 화자가 낸 안건은 기존처럼 소제목으로 분리. 대상: `app_dist/gemini_service.py`(`_SUMMARY_SPEAKER_TEMPLATE`, claude_service.py는 import 재사용이라 자동 반영), 모바일 `GeminiService.kt`(`SUMMARY_SPEAKER`, ChatGptService·ClaudeService도 이를 참조해 자동 반영), 캐노니컬 문서 `회의녹음요약_회의록템플릿.md` 양식 1. 검증: PC `ast.parse` 문법 통과, 모바일 `./gradlew compileDebugKotlin` BUILD SUCCESSFUL. **미해결**: PC exe 재빌드·모바일 APK 재빌드 미실시(소스만 반영, 실행 시 즉시 적용되나 배포 바이너리는 별도 빌드 필요), 실제 STT 텍스트로 재요약해 출력 형태를 육안 확인하는 절차는 미수행(프롬프트 규칙·예시만 검증). |
| **v4.0.2** | **화자별 턴 나열("라벨만 바뀐 Q&A") 금지 강화 + 일시 필드가 본문 날짜에 밀리는 버그 수정 + PC exe 재빌드 + 모바일 파일선택 소스 구분.** ① v4.0.1에서 Q&A 형식(`Q [화자]`/`A [화자]`)만 없앴을 뿐, org·topic·conference 3개 양식이 `**[화자] 의견**:`/`**답변**:`/`**제안**:`처럼 화자마다 줄을 나눠 나열하는 "라벨만 바뀐 대화체"로 새고 있던 것을 회원이 실제 소방훈련 논의 예시로 직접 지적 → 규칙을 "화자별 턴 나열 절대 금지 — 하나의 서술 문단으로 종합"으로 강화하고 나쁜 예/좋은 예를 프롬프트에 명시(PC·모바일 동일 반영). ② 전 양식의 "| 일 시 | {dt} |" 필드가 지정된 날짜 대신 본문 속 다른 날짜(예: "8월 23일 기준")로 잘못 채워지던 버그 발견·수정 — "이 값을 그대로 사용, 본문 속 다른 날짜와 혼동 금지" 명시. 기존 STT 텍스트로 재요약 검증해 화자별 나열 0건·일시 정확 반영 확인. ③ PC exe 재빌드 — `build_dist.bat`이 배치 파싱 오류(`^ was unexpected`)로 실패해 동일 PyInstaller 옵션을 직접 실행하는 방식으로 우회(`--noconfirm` 누락 시 기존 dist 폴더 때문에 COLLECT 단계에서 조용히 실패하고 exit 0으로 잘못 보고되는 것도 확인 — exe 타임스탬프로 실제 반영 여부 검증하는 습관 필요). ④ 모바일 v3.11.3(versionCode 48) — 녹음 화면 "파일 선택" 버튼이 앱 자체 녹음만 대상이던 것을, 소스 선택 다이얼로그를 앞에 붙여 🎙 앱 녹음 / 📞 통화녹음(`TPhoneCallRecords`) 중 고르게 함(기존 v3.11.0 `CallRecordingRepository`·`startCallSummary` 재사용, 중복 구현 없음). 실기기(SM-S938N) adb 설치 검증 완료. **미해결**: 통화녹음 선택 시 요약 양식이 기존처럼 전화통화메모로 자동 고정되어 파일선택 옆 요약방식 버튼 값이 반영되지 않음(회원 확인 후 필요 시 후속 조치). 상임위_20260825_회의록2 완전성(6월 노트 대비 분과 수 적음)은 회원이 앱에서 직접 재확인 예정. |
